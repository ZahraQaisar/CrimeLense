...
const AIInsightsPanel = () => {
  ...
  useEffect(() => {
    dataService.getModelMetadata()
      .then(data => setMetadata(data))
      .catch(() => {}); // graceful fallback to static data
  }, []);

  // Use real accuracy from metadata if available, else fallback to static
  const realAccuracy = metadata?.models?.binary_risk?.accuracy ?? 94;
  const realF1Cat    = metadata?.models?.crime_category?.f1_score
    ? Math.round(metadata.models.crime_category.f1_score * 100) : parseFloat(macroF1) * 100;
  ...
  const accCount  = useCountUp(realAccuracy, 1400);
  const f1CatCount = useCountUp(realF1Cat, 1400);
