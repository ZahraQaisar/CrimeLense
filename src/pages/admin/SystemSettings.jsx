      ...
      <div className="glass-panel rounded-2xl border border-danger/20 overflow-hidden">
        ...
        <div className="px-6 py-4 space-y-3">
          {dangerActions.map(({ key, label, desc, btn, icon: Icon }) => (
            <div key={key} className="flex items-center justify-between p-4 rounded-xl bg-danger/5 border border-danger/15">
              <div>
                <div className="text-sm font-semibold text-white">{label}</div>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg border border-danger/40 text-danger text-xs font-bold hover:bg-danger/10 transition-all shrink-0 ml-4"
              >
                <Icon size={13} /> {btn}
              </button>
            </div>
