...
// Crime trend data — different per range
const trendData = {
  '7d': [
    { day: 'Mon', crimes: 38 },
    { day: 'Tue', crimes: 52 },
...
const crimeTypeData = [
  { name: 'Theft',       value: 34 },
  { name: 'Assault',     value: 22 },
  { name: 'Robbery',     value: 18 },
  { name: 'Vandalism',   value: 14 },
  { name: 'Other',       value: 12 },
];
const DONUT_COLORS = ['#14F1D9', '#3B82F6', '#F59E0B', '#EF4444', '#8B5CF6'];

const hourlyData = [
  { hour: '00', crimes: 12 }, { hour: '02', crimes: 8  },
  { hour: '04', crimes: 5  }, { hour: '06', crimes: 9  },
  { hour: '08', crimes: 21 }, { hour: '10', crimes: 28 },
  { hour: '12', crimes: 35 }, { hour: '14', crimes: 31 },
  { hour: '16', crimes: 42 }, { hour: '18', crimes: 55 },
  { hour: '20', crimes: 67 }, { hour: '22', crimes: 48 },
];

const topAreas = [
  { rank: 1, name: 'Central Station',  incidents: 145, risk: 'High',   change: +12 },
  { rank: 2, name: 'Downtown Market',  incidents: 118, risk: 'High',   change: -4  },
  { rank: 3, name: 'West End Park',    incidents: 87,  risk: 'Medium', change: +6  },
  { rank: 4, name: 'Harbor District',  incidents: 64,  risk: 'Medium', change: -2  },
  { rank: 5, name: 'North Suburbs',    incidents: 39,  risk: 'Low',    change: +1  },
];
...
const RiskBadge = ({ risk }) => {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs font-bold border ${styles[risk]}`}>
      {risk}
    </span>
  );
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-safe/10 text-safe rounded-full border border-safe/20 text-sm font-bold w-fit">
            ...
            {['7d', '30d', '90d'].map(r => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all border ${
                  trendRange === r
                    ? 'bg-neon-teal/15 text-neon-teal border-neon-teal/30'
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={trendData[trendRange]} margin={{ top: 4, right: 8, left: -24, bottom: 0 }}>
              ...
              <Line
                ...
                stroke="#14F1D9"
                strokeWidth={2.5}
                dot={{ fill: '#14F1D9', r: 4, strokeWidth: 0 }}
                activeDot={{ r: 6, fill: '#14F1D9' }}
              />
            </LineChart>
          </ResponsiveContainer>
        </Panel>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <Panel title="Hourly Crime Activity" subtitle="Average crimes per hour of day">
            ...
            <BarChart data={hourlyData} margin={{ top: 4, right: 8, left: -24, bottom: 0 }}>
              ...
              <Bar dataKey="crimes" radius={[4, 4, 0, 0]}>
                {hourlyData.map((entry, i) => (
                  <Cell
                    key={i}
                    fill={entry.crimes >= 50 ? '#EF4444' : entry.crimes >= 30 ? '#F59E0B' : '#14F1D9'}
                  />
                ))}
              </Bar>
            </BarChart>
          ...
          <div className="flex gap-4 mt-3 justify-end">
            {[['#14F1D9', 'Low'], ['#F59E0B', 'Medium'], ['#EF4444', 'High']].map(([color, label]) => (
          ...
          <div className="space-y-2.5">
            {topAreas.map(area => (
              ...
              >
                ...
                {/* change indicator */}
                <div className={`flex items-center gap-0.5 text-xs font-bold shrink-0 ${area.change > 0 ? 'text-danger' : 'text-safe'}`}>
                  {area.change > 0
                    ? <TrendingUp size={13} />
        ...
        <Panel title="Recent System Logs">
          <div className="space-y-2">
            {[
              ...
              const dot = log.status === 'success' ? 'bg-safe' : log.status === 'warning' ? 'bg-warning' : 'bg-danger';
              return (
